...
addParamValue(ip, 'colormap', @boeBlueMap, @(x)isa(x,'function_handle')||ischar(x)||iscell(x)&&ischar(x{1}));
parse(ip, varargin{:});
results = ip.Results;
...
% Create colormap
ncolors = floor(length(prctiles)/2);  % /2, so the percentiles are finer
if iscell(cmapFun)
...
hold on;  lh = plot(xvals, centerline,'r-.', 'LineWidth', 2);
ph = flipud(ph);
...
function map = boeBlueMap(ncolors)

colors = [  
    25,25,112         % midnight blue	#191970   
    173,216,230       % light blue	#ADD8E6
    30,144,255        % dodger blue	#1E90FF
    135,206,235       % sky blue	#87CEEB
    0,191,255         % deep sky blue	#00BFFF
    95,158,160        % cadet blue  #5F9EA0
    70,130,180        % steel blue	#4682B4
    100,149,237       % corn flower blue	#6495ED
    65,105,225];      % royal blue	#4169E1
map = colors/256;
if nargin > 0
    map = interp1(1:size(map,1), map, 1:ncolors);
end
